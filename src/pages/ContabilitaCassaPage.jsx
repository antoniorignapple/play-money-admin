...
import {
  CalendarDays, Check, ChevronRight, Plus, RefreshCw, Search, Trash2, Wallet, X, AlertTriangle, ReceiptText,
} from 'lucide-react'
...
export default function ContabilitaCassaPage() {
  ...
  async function loadAll(silent = false) {
    ...
    try {
      ...
      const [detailRes, conteggiRes, overrideRes, venueRes, selectedRes] = await Promise.all([
        ...
        supabase
          .from('contabilita_cassa_debiti_selezionati')
          .select('conteggio_id')
          .eq('period_id', activePeriod.id),
      ])
  ...
  async function createTransfer() {
    ...
    setSaving(true)
    try {
      const { error } = await supabase.rpc('crea_trasferimento_cassa', {
        p_amount: amount,
      ...
      toast.error(`Movimento: ${e.message}`)
    } finally {
      setSaving(false)
    }
  }
        ...
        <div className="mx-auto max-w-[1320px] space-y-5 px-3 py-4 md:px-5">
          <div className="flex items-center justify-center py-1">
            <div className="inline-flex items-center gap-3 rounded-full border border-[#d9b45f]/35 bg-white/70 px-5 py-2.5 shadow-[0_10px_30px_-22px_rgba(95,64,11,.55)] backdrop-blur-sm">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#f4e7c2] text-[#98701f]"><Wallet size={15}/></div>
              <h1 className="text-[15px] font-black uppercase tracking-[0.20em] text-[#3d2a0b] md:text-[17px]">Contabilità Cassa</h1>
            ...
            <>
              ...
              <section className="overflow-hidden rounded-[24px] border border-black/8 bg-white shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-black/7 px-5 py-4">
                  <div>
                    <h2 className="text-[14px] font-black uppercase tracking-[0.12em] text-[#3d2a0b]">Movimenti Contabilità Cassa</h2>
                    <p className="mt-1 text-[11px] text-black/45">Movimenti amministrativi registrati nel periodo attivo.</p>
                  </div>
                ...
                <div className="divide-y divide-black/6">
                  {loading ? (
                    ...
                    <div className="p-10 text-center">
                      ...
                      <p className="mt-3 text-[13px] font-bold text-black/55">Nessun movimento amministrativo</p>
                    </div>
                  ) : transfers.map((row, index) => (
                    <div key={row.id} className="grid grid-cols-[86px_1fr_auto_auto] items-center gap-3 px-5 py-3.5 md:grid-cols-[115px_1fr_180px_42px]">
                      <p className="text-[10px] font-bold tabular-nums text-black/45">{fmtDate(row.transfer_date)}</p>
                      <div className="min-w-0">
                        <p className="truncate text-[12px] font-black uppercase tracking-[.02em] text-[#33250f]">{row.destination}</p>
                        {row.note && <p className="mt-0.5 whitespace-pre-wrap text-[10px] text-black/45">{row.note}</p>}
                      </div>
                      <p className="text-right text-[15px] font-black tabular-nums text-[#33250f]">{euro(row.amount)}</p>
                      <button onClick={() => setDeleteRow(row)} title="Elimina"
          ...
          <div className="space-y-4">
            ...
            <Field label="Descrizione">
              <input value={form.destination} onChange={(e) => setForm({ ...form, destination: e.target.value })}
                className="h-11 w-full rounded-[14px] border border-black/10 bg-[#faf9f5] px-4 text-[13px] font-bold outline-none focus:border-[#c9a348]" placeholder="Es. Acconto generale N.1"/>
            </Field>
            <Field label="Data">
