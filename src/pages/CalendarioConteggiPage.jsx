...
export default function CalendarioConteggiPage() {
  ...
  const selectedCount = useMemo(
    () => [...selected].filter((date) => date.startsWith(`${year}-${pad(month + 1)}`)).length,
    [selected, year, month],
  )
  ...
  return (
    <PageLayout>
      <PageHeader
        title="CALENDARIO CONTEGGI"
        subtitle="Programma i giorni di conteggio visibili nell’app Dipendenti"
        actions={(
          <>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      I giorni rossi sono festivi o domeniche e non possono essere selezionati.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 self-start rounded-2xl border border-amber-200 bg-white/85 px-4 py-3 shadow-sm md:self-auto">
                  <div>
                    ...
                    <p className="mt-0.5 text-2xl font-black text-slate-950">{selectedCount}</p>
                  </div>
            ...
            <div className="p-4 md:p-7">
              <div className="overflow-hidden rounded-[24px] border border-amber-200/80 bg-white shadow-[0_18px_45px_-32px_rgba(15,23,42,0.6)]">
                ...
                <div className="grid grid-cols-7 bg-amber-100/80 gap-px">
                  {cells.map((day, index) => {
                    if (!day) {
                      return <div key={`empty-${index}`} className="min-h-20 bg-slate-50/70 md:min-h-24" />
                    }
                    ...
                    return (
                      <button
                        ...
                        onClick={() => toggle(day)}
                        disabled={blocked}
                        title={holidayName || (sunday ? 'Domenica' : active ? 'Giorno di conteggio' : 'Giorno lavorativo')}
