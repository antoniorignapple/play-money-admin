...
export default function ProgrammazioneConteggi({ dates, monthKey }) {
  ...
  return <>
    ...
    {editor && <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/65 p-3 backdrop-blur-sm" onClick={() => !saving && setEditor(null)}><div className="flex max-h-[94vh] w-full max-w-[820px] flex-col overflow-hidden rounded-[28px] border border-[#d6b76f] bg-[#fffdf9] shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-4 overflow-y-auto p-4">
        ...
        <div><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-black text-[#805718]">LOCALI E ORDINE TAPPE</p><span className="rounded-full bg-amber-100 px-3 py-1 text-[10px] font-black text-amber-800">{editor.venueIds.length} SELEZIONATI</span></div>
          ...
          <div className="grid max-h-[390px] gap-2 overflow-y-auto md:grid-cols-2">{data.venues.filter((v) => !String(v.id).toUpperCase().startsWith('D')).filter((v) => { const q = editorSearch.trim().toLowerCase(); return !q || `${v.id} ${v.name} ${v.city || ''} ${v.code || ''}`.toLowerCase().includes(q) }).sort((a, b) => { const giroVenueIds = new Set(data.giroVenues.filter((x) => String(x.giro_id) === String(editor.giroId)).map((x) => String(x.venue_id))); const ag = giroVenueIds.has(String(a.id)) ? 0 : 1; const bg = giroVenueIds.has(String(b.id)) ? 0 : 1; return ag - bg || String(a.name).localeCompare(String(b.name), 'it'); }).map((venue) => { const index = editor.venueIds.indexOf(venue.id); const checked = index >= 0; const counted = data.countedVenueIds.includes(String(venue.id)); const locked = counted && !checked; return <div key={venue.id} className={`flex items-center gap-2 rounded-[15px] border p-2 ${counted ? 'border-emerald-200 bg-emerald-50/70' : checked ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white'}`}><button disabled={locked} onClick={() => toggleVenue(venue.id)} className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border disabled:cursor-not-allowed ${counted ? 'border-emerald-600 bg-emerald-600 text-white' : checked ? 'border-amber-700 bg-amber-700 text-white' : 'border-slate-300 text-transparent'}`}><Check size={14}/></button><button disabled={locked} onClick={() => toggleVenue(venue.id)} className="min-w-0 flex-1 text-left disabled:cursor-not-allowed"><span className={`block truncate text-[11px] font-black ${counted ? 'text-emerald-800' : 'text-slate-900'}`}>{venue.name}</span><span className={`mt-0.5 flex items-center gap-1 truncate text-[9px] font-bold ${counted ? 'text-emerald-600' : 'text-slate-400'}`}>{counted ? <><Check size={9}/>GIÀ CONTEGGIATO NEL PERIODO</> : <><MapPin size={9}/>{venue.city || 'Città non indicata'}</>}</span></button>{checked && !counted && <><span className="min-w-6 text-center text-[10px] font-black text-amber-700">{index + 1}</span><div className="grid"><button onClick={() => moveVenue(index, -1)} disabled={index === 0} className="text-amber-800 disabled:opacity-20"><ChevronUp size={15}/></button><button onClick={() => moveVenue(index, 1)} disabled={index === editor.venueIds.length - 1} className="text-amber-800 disabled:opacity-20"><ChevronDown size={15}/></button></div></>}</div>})}</div>
        </div>
      </div>
