...
async function generateConteggiPdf({
  ...
  const drawSummary = (y) => {
    ...
    drawFinalBadge(boxX + normalW * 5, y, finalW, boxH, riepilogo.finale);
  };
    ...
    for (let p = 0; p < totalTablePages; p++) {
      ...
      pageRows.forEach((r, idx) => {
        ...
        values.forEach((val, i) => {
          ...
          const text =
            i === 0 ? doc.splitTextToSize(val, col.w - 8)[0] || "" : val;
